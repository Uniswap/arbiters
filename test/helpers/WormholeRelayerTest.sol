...
/**
 * @notice This file is a copy of lib/wormhole-solidity-sdk/src/testing/WormholeRelayerTest.sol
 * @dev Copied due to stack too deep errors and to adjust import paths for our project structure.
 *
 * Original source: https://github.com/wormhole-foundation/wormhole-solidity-sdk
 *
 * This is a testing harness for simulating cross-chain Wormhole relayer interactions.
 * It creates multiple blockchain forks and simulates message passing between them.
 */
...
import "../../lib/wormhole-solidity-sdk/src/interfaces/CCTPInterfaces/IMessageTransmitter.sol";
import "../../lib/wormhole-solidity-sdk/src/interfaces/CCTPInterfaces/ITokenMessenger.sol";
import "../../lib/wormhole-solidity-sdk/src/Utils.sol";

import "../../lib/wormhole-solidity-sdk/src/testing/helpers/WormholeSimulator.sol";
import "../../lib/wormhole-solidity-sdk/src/testing/helpers/CircleCCTPSimulator.sol";
import "../../lib/wormhole-solidity-sdk/src/testing/ERC20Mock.sol";
...
import "./MockOffchainRelayer.sol";
...
abstract contract WormholeRelayerTest is Test {
    ...
    constructor() {
        ...
        forks[0] = chainInfosMainnet[30]; // base
    ...
    function _setUp() internal {
        ...
        for (uint256 i = 0; i < activeForksList.length; ++i) {
            ...
            fork.guardian = new WormholeSimulator(address(fork.wormhole), DEVNET_GUARDIAN_PK);
            fork.circleAttester =
                new CircleMessageTransmitterSimulator(address(fork.circleMessageTransmitter), CIRCLE_ATTESTER_PK);
        }
        ...
        mockOffchainRelayer = new MockOffchainRelayer(
            address(firstFork.wormhole), address(firstFork.guardian), address(firstFork.circleAttester)
        );
    ...
    function initChainInfo() private {
        ...
        chainInfosTestnet[4] = ChainInfo({
            chainId: 4,
            name: "bsc testnet",
            url: vm.envOr("BSC_TESTNET_RPC_URL", string("https://bsc-testnet-rpc.publicnode.com/")),
        ...
        chainInfosTestnet[5] = ChainInfo({
            ...
            circleMessageTransmitter: IMessageTransmitter(0xe09A679F56207EF33F5b9d8fb4499Ec00792eA73),
        ...
        chainInfosMainnet[30] = ChainInfo({
            ...
            circleMessageTransmitter: IMessageTransmitter(address(0xAD09780d193884d503182aD4588450C416D6F9D4)),
            circleTokenMessenger: ITokenMessenger(address(0x1682Ae6375C4E4A97e4B583BC394c861A46D8962)),
...
abstract contract WormholeRelayerBasicTest is WormholeRelayerTest {
    ...
    constructor() WormholeRelayerTest() {
        setMainnetForkChains(30, 23); // base and arbitrum
    }
