...
abstract contract ExecutorSendImpl is ExecutorSharedBase {
    ...
    function _publishAndRelay(
        ...
        unchecked {
            // TODO: decide on errors for message fees
            ...
            _executor.requestExecution{
                value: totalCost - messageFee
            }(
                peerChain,
                peerAddress,
                refundAddress,
...
abstract contract ExecutorReceiveImpl is ExecutorSharedBase, IVaaV1Receiver {
    ...
    function executeVAAv1(bytes calldata multiSigVaa) external payable virtual {
        ...
        _executeVaa(payload, timestamp, nonce, emitterChainId, emitterAddress, sequence, consistencyLevel);
    }
}

abstract contract ExecutorSend is ExecutorSharedBase, ExecutorSendImpl {
    constructor(address coreBridge, address executor) ExecutorSharedBase(coreBridge) ExecutorSendImpl(executor) {}
}

abstract contract ExecutorReceive is ExecutorSharedBase, ExecutorReceiveImpl {
    constructor(address coreBridge) ExecutorSharedBase(coreBridge) ExecutorReceiveImpl(coreBridge) {}
}
